...
本项目是一个 Codex skill，用于启动和执行需求开发工作流。它要求先完成 Codex 配置检查与自动修复、项目说明与 Git 基线检查，再进行需求澄清、需求归档、分支开发、测试报告、选择框确认、收尾确认和最终提交。
...
18. 如果用户通过选择框明确结束本次开发，在当前需求分支提交需求归档、实现代码、测试报告和相关测试资产。
...
提交信息优先遵循项目内已有规范或历史提交风格。没有明确规范时，使用简洁的 Conventional Commit：
